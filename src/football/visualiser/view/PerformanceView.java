...
public class PerformanceView{
    ...
    public double getPossessionTimes(Map<Integer, Double> individualPossessionTimes){
        ...
        for (Map.Entry<Integer, Double> entry : individualPossessionTimes.entrySet()) {
            ...
            if(key == playerID){
                // convert it from millionsecond into second
                possessionTimes = entry.getValue() / 1000;

                break;
            }
        }
        // keep two decimal for the data
        BigDecimal b = new BigDecimal(possessionTimes);
        double finalPossessionTimes = b.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();

        return finalPossessionTimes;
    }
